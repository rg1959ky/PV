Operation =1
Option =0
Where ="(((TransactionLines.[Order Type])=\"509\") And ((TransactionLines![Orders Transa"
    "ction Inventory Quantity]*ItemCosts![Last Purchase Price-Price(BC)])>Forms![MO O"
    "verissue Audit Parameters]!txtThreshhold) And ((FromERPDateFormat(TransactionLin"
    "es!Date))>=CDate(Forms![MO Overissue Audit Parameters]!txtDateStart) And (FromER"
    "PDateFormat(TransactionLines!Date))<=CDate(Forms![MO Overissue Audit Parameters]"
    "!txtEndDate)))"
Begin InputTables
